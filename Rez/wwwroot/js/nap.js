...
async function nap() {
    return await Promise.all(
        c.map((v, i) => {
            ...
            arr[i].forEach(h => {
                tinh.QuanHuyen.push({Ten: h});
            })

            let data = JSON.stringify(tinh);


            return fetch('http://localhost:5094/api/tinh', {
